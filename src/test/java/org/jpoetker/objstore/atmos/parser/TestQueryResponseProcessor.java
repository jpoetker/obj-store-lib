...
public class TestQueryResponseProcessor {
	...
	public void testParseObjectInfo() throws Exception {
		...
		Iterator<Metadata> userMetadata = first.getUserMetadata().iterator();
		assertThat(userMetadata.next().isListable(), is(false));
		assertThat(userMetadata.next().isListable(), is(false));
		assertThat(userMetadata.next().isListable(), is(true));
	}
}
